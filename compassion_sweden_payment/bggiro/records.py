...
from .enums import PaymentStatus, PeriodCode, TransactionType
...
class PaymentRecord(Record):
    ...
    payment_status_code: Optional["PaymentStatus"] = field(
        default=None, converter=to_payment_status
    )
    ...
    def to_ocr(self) -> str:
        ...
        return (
            ...
            + (
                isinstance(self.payment_date, str)
                and f"{self.payment_date}"
                or f"{self.payment_date:%Y%m%d}"
            )
            + f"{self.period_code:01d}"
            + (
                self.number_recurring_payments
                and f"{self.number_recurring_payments:03d}"
                or (" " * 3)
            )
            + " "
            ...
            + (
                ...
                and f"{self.payment_status_code:01d}"
                or (" " * 1)
            )
        )
...
class EndRecord(Record):
    ...
    def to_ocr(self) -> str:
        ...
        return (
            f"09"
            f"{self.date_written:%Y%m%d}"
            f"9900" + (" " * 14) + f"{self.total_amount_outgoing:012d}"
...
def parse(data: str) -> list[R]:
    ...
    record_classes = {
        TransactionType.INCOMING_PAYMENT: PaymentRecord,
