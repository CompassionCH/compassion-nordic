=================================
Completion rules for Swedish bank
=================================
...
Adds completion rules for swish and other swedish statements.
...
Make sure to set your similarity_threshold high sufficient to avoid
finding partner names that don't really match the bank statement data.
(0.3 is the default and could produce false positives).

On Postgres with root access we could set it with : `ALTER SYSTEM SET pg_trgm.similarity_threshold = 0.8;`
or by setting it in the `postgresql.conf` file.
