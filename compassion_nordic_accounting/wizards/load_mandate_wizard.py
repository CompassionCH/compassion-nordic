...
class LoadMandateWizard(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", "Partner", compute="_compute_all", store=True
    )
    ...
    kid = fields.Char(
        help="kundeidentifikasjon, is the payment reference for a specific partner"
    )
    ...
    def _compute_all(self):
        for rec in self:
            rec.partner_id = (
                rec.mandate_id.partner_id if rec.mandate_id else rec.partner_id
            )
            rec.company_id = (
                rec.mandate_id.company_id if rec.mandate_id else rec.company_id
            )
            if rec.mandate_id:
        ...
        for vals in vals_list:
            ...
            if mandate.is_cancelled:
                user_id = (
                    ...
                    .sudo()
                    .get_param(
                        f"compassion_nordic_accounting.mandate_notif_{self.env.company.id}"
                    )
                )
                self.env["account.banking.mandate"].browse(
                    vals.get("mandate_id")
                ).activity_schedule(
                    activity_type_id=self.env["mail.activity.type"]
