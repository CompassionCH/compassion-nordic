...
class AccountStatementImport(models.TransientModel):
    ...
    large_file_import = fields.Boolean(
        help="Use this for large statement files. The process will run in the "
        "background so that you can continue to work in the meantime.",
        default=False,
    )
    maximum_lines = fields.Integer(
        help="Use this to split large statements into multiple smaller ones."
        "It can be useful for speeding up "
    ...
    def _split_statement(self, stmts_vals):
        ...
        for chunk in numpy.array_split(lines, (len(lines) // self.maximum_lines) or 1):
            ...
            balance_end = balance_start + sum(
                map(lambda line: float(line["amount"]), chunk)
            )
