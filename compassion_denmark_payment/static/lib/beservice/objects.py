...
from .enums import DeliveryType, SectionType, SignCode, TransactionCode
from .records import (
...
class DataDelivery(ABC):
    ...
    delivery_date: Optional[datetime.date] = field(
        default=None, converter=to_date_or_none
    )
    ...
    def _get_sections(records: list[R]) -> list["Section"]:
        ...
        for record in records:
            if isinstance(record, SectionStartRecord):
                ...
                temp_section_record.append(record)
        if temp_section_record:
            raise ValueError("Expected Section End record at the end")
...
class DataDeliveryCollection(DataDelivery):
    def add_section(
        self,
        pbs_number: int,
        debtor_group_number: int,
        data_supplier_id: str,
        main_text_line: Optional[str] = None,
    ) -> "CollectionSection":
        section = CollectionSection(
            pbs_number=pbs_number,
...
class Section(ABC):
    ...
    def _parse_text_line(
        ...
        for text_line in text_lines:
            ...
            parsed_text_lines[rec_num].append(
                text_line[1][:60].rsplit(" ", 1)
                if text_line[1][:60] != text_line[1]
                else text_line[1]
            )
    ...
    def get_num_of_record_52_62(self):
        return sum(
            [
                information.get_num_of_record_52_62()
                for information in self.information_list
            ]
        )
...
class Collection(InformationData):
    ...
    text_lines: OrderedDictType[int, list[str]] = field(
        default=Factory(OrderedDictType), repr=False
    )
...
class PaymentInformation(InformationData):
    ...
    payment_date: Optional["datetime.date"] = field(
        default=None, converter=to_date_or_none
    )
    bookkeping_date: Optional["datetime.date"] = field(
        default=None, converter=to_date_or_none
    )
    ...
    def from_records(cls, records: list[R]) -> "PaymentInformation":
        ...
        return cls(
            ...
            payment_amount=int(record.payment_amount / 100)
    ...
    def to_records(self) -> Iterable["Record"]:
        ...
        yield InfoRecord(
            ...
            payment_amount=self.payment_amount * 100
