...
from .enums import DeliveryType, RecordType, SectionType, SignCode, TransactionCode
...
class SectionStartRecord(Record):
    ...
    def to_ocr(self) -> str:
        """Get record as OCR string."""
        common_fields = "BS012" + f"{self.pbs_number:08d}" + f"{self.section_type:04d}"
        if self.section_type == SectionType.COLLECTION:
            section_fields = (
                " " * 5
                + f"{self.debtor_group_number:05d}"
                + (
                    self.data_supplier_id
                    and f"{self.data_supplier_id:15}"
                    or (" " * 15)
                )
                + (" " * 4)
            ...
            section_fields = (
                (
                    " "
                    if self.section_type == SectionType.REGISTERED_AND_CANCELLED_MANDATE
                    else "0"
                )
                * 3
                + f"{self.debtor_group_number:05d}"
...
class InfoRecord(Record):
    ...
    def to_ocr(self) -> str:
        """Get record as OCR string."""
        common_part = (
            "BS042" + f"{self.pbs_number:08d}" + f"{self.transaction_code:04d}"
        )
        if self.transaction_code == TransactionCode.COLLECTION_INFORMATION:
            ...
            transaction_spec = (
                ...
                + (
                    self.bookkeping_date
                    and f"{self.bookkeping_date:%d%m%y}"
                    or ("0" * 6)
                )
...
class SectionEndRecord(Record):
    ...
    def to_ocr(self) -> str:
        """Get record as OCR string."""
        common_part = "BS092" + f"{self.pbs_number:08d}" + f"{self.section_type:04d}"
        if self.section_type == SectionType.COLLECTION:
            section_spec = (
                "00000"
                + f"{self.debtor_group_number:05d}"
