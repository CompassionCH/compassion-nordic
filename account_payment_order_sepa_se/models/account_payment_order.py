...
class AccountPaymentOrder(models.Model):
        ...
        if initiating_party_identifier:
            ...
            if initiating_party_scheme:
                ...
                iniparty_org_other_scheme_name = etree.SubElement(
                    iniparty_org_other_scheme,
                    "Cd",  # Used to be Prtry but the SEB parser
                    # gave an error and wanted Cd instead
                )
    ...
    def generate_party_agent(
        ...
        if partner_bank.bank_bic:
            ...
            # New Comment: Seem like if if we have a ban giro account
            # then we still need the 'Creditor Agent' block which is why i have added
            # or order == "C" and re.match('\d{3,4}-\d{4}', partner_bank.acc_number)):
            # to the if case
            if order == "B" or (
                order == "C"
                and gen_args["payment_method"] == "DD"
                or order == "C"
                ...
                if partner_bank.acc_number and re.match(
                    r"\d{3,4}-\d{4}$", partner_bank.acc_number
                ):  # ADDIDTIONS
    ...
    def generate_party_acc_number(
        ...
        # ADDIDTIONS
            ...
            if (
                ...
                and re.match(r"\d{3,4}-\d{4}$", partner_bank.acc_number)
                or partner_bank.acc_type == "bank_giro"
            ):
                party_account_other_schmenm = etree.SubElement(
                    party_account_other, "SchmeNm"
                )
                party_account_other_cd = etree.SubElement(
                    party_account_other_schmenm, "Prtry"
                )
                ...
                party_account_other_cd = etree.SubElement(
                    party_account_other_schmenm, "Cd"
                )
