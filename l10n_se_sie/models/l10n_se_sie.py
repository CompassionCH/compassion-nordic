...
class account_sie(models.TransientModel):
    ...
    def check_import_file(self, data=None, check_periods=True):
        ...
        if data or self.data:  # IMPORT TRIGGERED
            ...
            if check_periods:
                ...
                if missing_period:
                    raise Warning(
                        "Missing period/fiscal year for %s - %s."
                        % (missing_period[0], missing_period[1])
                    )
            return checked
    ...
    def send_form(self):
        ...
        if self.data:  # IMPORT TRIGGERED
            if not self.move_journal_id:
                raise Warning("Please select a journal")
            data = self.cleanse_with_fire(self.data)
            if not self.check_import_file(data):
                missing_accounts = self.env["account.account"].check__missing_accounts(
                    self._import_accounts(data)
            ...
            if self.account_ids:
                accounts = [
                    l.move_id.id
                    for l in self.env["account.move.line"].search(
    ...
    def make_sie(self, ver_ids):
        ...
        for fiscalyear in self.fiscalyear_ids:
            ...
            str += "#RAR %s %s %s\n" % (
                self._get_rar_code(fiscalyear),
                fiscalyear.date_from.strftime("%Y%m%d"),
                fiscalyear.date_to.strftime("%Y%m%d"),
            )
            if len(self.fiscalyear_ids) == 1:
                str += "#RAR %s %s %s\n" % (
                    -1,
                    (fiscalyear.date_from - relativedelta(years=1)).strftime("%Y%m%d"),
        ...
        for ver in ver_ids.sorted(lambda r: r.date, reverse=False):
            ...
            rar = (
                0
                if len(self.fiscalyear_ids) == 1
                else self._get_rar_code(
                    self.fiscalyear_ids.filtered(
                        lambda x: x.date_from <= ver.date and x.date_to >= ver.date
                    )
                )
            )
            if old_rar != False and old_rar != rar:
                for account in ub:
                    ...
                    if bsacc:
                        str += "#UB %s %s %s\n" % (
                            old_rar,
                            self.escape_sie_string(account),
                            ub.get(account, 0.0),
                        )
                    else:
            ...
            old_rar = rar
            str += '#VER %s "%s" %s "%s" %s %s\n{\n' % (
                self.escape_sie_string(ver.journal_id.type),
                ver.id,
            ...
            for trans in ver.line_ids:
                ...
                if (
                    trans.display_type == "line_note"
                    or trans.display_type == "line_section"
                ):
                    continue
                str += '#TRANS %s {} %s %s "%s" %s %s\n' % (
                    self.escape_sie_string(trans.account_id.code),
                    trans.debit - trans.credit,
                    self.escape_sie_string(trans.date.strftime("%Y%m%d")),
                    self.escape_sie_string(self.fix_empty(trans.name)),
                    trans.quantity,
    ...
    def export_sie(self, ver_ids):
        ...
        sie_form.write(
            {
                "state": "get",
                "data": base64.b64encode(sie_form.make_sie(ver_ids)),
                "filename": "filename.se",
            }
        )
        ...
        _logger.info(
            ...
            % (
                ...
                sie_form,
                sie_form.sie_file,
                base64.b64encode(sie_form.make_sie(ver_ids)),
            )
        )
            ...
            if line["label"] == "#VER":
                try:
                    ...
                    self.env["account.period"].search([], limit=1).find(
                        dt=line[3], company_id=self.company_id.id
                    )  # The find method has self.ensure_one, which is why i find one record.
    ...
    def _import_ver(self, data):
        ...
        for line in data:
            if line["label"] == "#VER":
                ...
                list_ref = (
                    line.get(1, " ") + " " + line.get(2, " ") + " " + line.get(4, " ")
                )  # reference
                ...
                if len(serie_to_journal_lines) > 1:
                    serie_to_journal_lines_warning = (
                        "There are two lines the same series.\n"
                    )
                ...
                ver_id = self.env["account.move"].create(
                    {
                        ...
                        .search([], limit=1)
                        .find(dt=list_date, company_id=self.company_id.id)
                        .id,
                ...
                for l in line.get("lines", []):
                    if l["label"] == "#TRANS":
                        ...
                        for tag_type, tag_val in trans_object:
                            tag_name_prefix = (
                                f"{tag_val} %"
                            )  # Equivalent to 'regex': "^{tag_val} .*$"
                            if tag_name_prefix not in tag_table:
                                matching_tag = tag_model.search(
                                    [("name", "=like", tag_name_prefix)]
                                )
                        ...
                        trans_id = (
                            self.with_context(context_copy)
                ...
                period_id = (
                    ...
                    .search(
                        [
                            ("date_start", "=", first_date_of_year),
                            ("date_stop", "=", first_date_of_year),
                            ("company_id", "=", self.company_id.id),
                            ("special", "=", True),
                        ]
                    )
                ...
                ib_qnt = line.get(
                    4
                )  # We already have a amount, what is the purpose of having a quantity as well
        ...
        if ib_move_id:
            ...
            if move_balance != 0:
                line_vals = {
                    ...
                    # If ib_amount is negativ then we create a credit line in the account move otherwise a debit line
                    "debit": float(move_balance) < 0
                    and float(move_balance) * -1
                    or 0.0
                    or 0.0,
                    # 'period_id': period_id,
                    "date": first_date_of_year,
                ...
                self.with_context(context_copy).env["account.move.line"].create(
                    line_vals
                )
